...
function ControlsDropdown(props){
    return(
            <div className="bg-zmapmenu h-auto w-[95%] ml-16 mt-11 rounded-md flex flex-col p-2 gap-2 absolute" hidden={props.hidden}>
            <div className="flex justify-end"><Cross className="h-5 w-5"/></div>
            {props.headingdiv}
            <Rightbarbtn text="Print Pdf" icon={<Print className="h-5 w-5 fill-white"/>}/>
            </div>
    )
}
